...
def main():
    ...
    is_palette = None
    specified_palette: list[str] = is_palette if is_palette is not None else []
    ...
    n_workers = 0
    num_workers = cpu_count() if n_workers == 0 else n_workers
    ...
    with logging_redirect_tqdm():
        with tqdm(filenames, desc='Processing images', unit='images') as pbar:
            # print(filenames)
            # Convert the list of filenames to Path objects
            filenames = [Path(f) for f in filenames]
            # filenames = Path(filenames).stem
